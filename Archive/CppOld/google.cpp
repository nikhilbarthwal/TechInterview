...
int main(void)
{
    ...
    for(int j=5;j<300;j+=2)                    // Run a loop for all remaining odd no.
    {
        q=0;
        for(int k=0;k<n;k++)                // Look up all previous prime no. (Except 2)
            if (3*p[k]>j) k=n;            // If the j > p[k]/3 -> no need to check further
            else if (j%p[k]==0) { q=1;k=n; }     // Check if j is prime
